...
class LimitedLengthRandomSampler(Sampler):
    def __init__(self, data_source, *, num_samples: int, shuffle=True):
        super().__init__(data_source)
        self.data_source = data_source
        self.shuffle = shuffle
        self._num_samples = num_samples
    ...
    def __iter__(self):
        total = 0
        try:
            for i in _InfiniteRandomIterator(self.data_source, shuffle=self.shuffle):
...
if __name__ == '__main__':
    sampler = LimitedLengthRandomSampler([123,], num_samples=1000000000000000000000000000000, shuffle=True)
